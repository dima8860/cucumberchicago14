...
import API_Models.Student.Address;
...
public class StudentsListAPI {
    ...
    @Then("user searches for the student with student ID {string}")
    public void user_searches_for_the_student_with_student_ID(String studentID) throws InterruptedException {
    ...
    public void user_verifies_that_no_result_should_show() {
        int actualSize = cbtHomePage.searchResultsName.size();
        Assert.assertEquals("Deleting student failed", 0, actualSize);
    }
    ...
    public void user_searches_for_the_student_with_newly_created_student_ID() throws InterruptedException {
        cbtHomePage.studentsDropdown.click();
        cbtHomePage.allStudentsLink.click();
        Thread.sleep(500);
